...
# ## Prevalencia DM por grupos quinquenales de edad +35
# prev_dm_ge5 <- read_csv("Bases de datos/clean/arg_prev_dm_35.csv")

## Prevalencia DM por grupos decenales de edad
prev_dm_ge10 <- read_csv("Bases de datos/clean/arg_prev_dm_ge10.csv")

## Prevalencia DM por grupos decenales de edad y región
prev_dm_ge10_reg <- read_csv("Bases de datos/clean/arg_prev_dm_ge10_reg.csv")

## Secuelas DM2 (datos tiemporales 2014)
comp_dm <- read_csv("Bases de datos/clean/frec_DW_complic_sexo_ge10.csv")


# ## AVP por grupos quinquenales de edad
# AVP_ge5 <- read_csv("Bases de datos/clean/arg_defun_avp_ge5.csv")

# ## AVP por grupos quinquenales de edad +35
# AVP_ge5 <- read_csv("Bases de datos/clean/arg_defun_avp_35.csv")

## AVP por grupos decenales de edad
AVP_ge10 <- read_csv("Bases de datos/clean/arg_defun_avp_30.csv")

## AVP por grupos decenales de edad y región
AVP_ge10_reg <- read_csv("Bases de datos/clean/arg_defun_avp_30_reg.csv")
...
proy_ge10 <- read_csv("Bases de datos/clean/arg_proy_2005_2018_ge10.csv")

## Proyecciones INDEC 2005-2018 por grupos decenales de edad y región
proy_ge10_reg <- read_csv("Bases de datos/clean/arg_proy_2005_2018_ge10_reg.csv")
...
tasas_nac_ge10_aj <- AVAD_ge10 %>%
  ...
  summarise(across(.cols = c(AVAD_n_est, AVP_n_est, AVD_n_est),
                .fns = list(
                  abs = ~ sum(.x)),
  ...
  group_by(anio_enfr, sexo) %>% 
  mutate(across(.cols = c(AVAD_est_sum, AVP_est_sum, AVD_est_sum),
                   .fns = list(
...
tasas_nac_ge10_aj |> 
  pivot_longer(cols = c(AVD_tasa_aj, AVP_tasa_aj, AVAD_tasa_aj), names_to = "indicador", values_to = "tasas") %>% 
  ...
  scale_colour_manual(values = c("#F2BA52",
                                 "#F2B3B3",
                                 "#B0D9BA"),
                      labels = c("AVAD_tasa_aj" = "AVAD",
                                 "AVD_tasa_aj" = "AVD",
                                 "AVP_tasa_aj" = "AVP"))
...
tasas_nac_ge10 |> 
  pivot_longer(cols = c(AVD_tasa_gral, AVP_tasa_gral),
               names_to = "indicador",
               values_to = "valor") %>% 
...
tasas_prov_ge10_aj |> 
  filter(sexo == "Mujer") %>% 
  pivot_longer(cols = c(AVAD_tasa_aj, AVD_tasa_aj, AVP_tasa_aj), names_to = "indicador", values_to = "tasas") %>% 
...
AVAD_ge10_reg %>%
  ...
  group_by(anio_enfr, region, sexo) %>% 
  summarise(AVP_prop = round(AVP/AVAD*100, digits = 2),
