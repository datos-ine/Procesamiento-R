...
qualidiab_2014 <- qualidiab_2014 %>%   
  mutate(across(.cols = starts_with(c("cobertura", "complicaciones", "ojos")),
         ...
         grupedad_5 = as_factor(case_when(
         ...
         grupedad_10 = as_factor(case_when(
           edad >= 0 & edad < 10 ~ "01_0 a 9",
           edad >= 10 & edad < 20 ~ "02_10 a 19",
           edad >= 20 & edad < 30 ~ "03_20 a 29",
           edad >= 30 & edad < 40 ~ "04_30 a 39",
           edad >= 40 & edad < 50 ~ "05_40 a 49",
           edad >= 50 & edad < 60 ~ "06_50 a 59",
           edad >= 60 & edad < 70 ~ "07_60 a 69",
           edad >= 70 & edad < 80 ~ "08_70 a 79",
           edad >= 80 ~ "09_80 y más",
           .default = NA)),
         
         grupedad_gr = as_factor(case_when(
...
qualidiab_2014 <- qualidiab_2014 %>% 
  mutate(grupedad_5 = factor(grupedad_5, levels = c(
    "01_0 a 4",
    "02_5 a 9",
    "03_10 a 14",
    "04_15 a 19",
    ...
    "17_80 y más")),
         ...
         grupedad_10 = factor(grupedad_10, levels = c(
           "01_0 a 9",
           "02_10 a 19",
           "03_20 a 29",
           "04_30 a 39",
    ...
    grupedad_gr = factor(grupedad_gr, levels = c(
      "01_0 a 39",
      "02_40 a 59",
      "03_60 a 80",
      "04_80 y más")))
...
qualidiab_2014_dm2 %>%
  ...
  ggplot(aes(x = grupedad_10, fill = presencia)) +
...
qualidiab_2014_dm2 %>%   
  ...
  select(c(sexo, edad, grupedad_10, 
...
complic_frecyDW_10 <- qualidiab_2014_dm2 %>% 
  ...
  pivot_longer(cols = 3:14, names_to = "compli", values_to = "presencia") %>%    
  
  mutate(compli = case_when(
    compli == "complic" ~ "Sin complicaciones",
    compli == "complicaciones_iam_c" ~ "IAM",
    compli == "complicaciones_ic" ~ "IC",
    compli == "complicaciones_acv" ~ "ACV",
    compli == "complicaciones_claudicacion_miembros_inferiores" ~ "Claudicación miembros inferiores",
    compli == "ojos_retinopatia_no_proliferativa" ~ "Retinopatía no proliferativa",
    compli == "ojos_retinopatia_proliferativa" ~ "Retinopatía proliferativa",
    compli == "complicaciones_ceguera" ~ "Ceguera", 
    compli == "complicaciones_nefropatia_c" ~ "Nefropatía", 
    compli == "complicaciones_neuropatia_periferica_c" ~ "Neuropatía periférica",
    compli == "complicaciones_amputacion" ~ "Amputación",
    compli == "complicaciones_disfuncion_erectil" ~ "Disfunción eréctil"),   
    
    compli = factor(compli, levels = c(     
      "Sin complicaciones",
      "IAM",
      "IC",
      "ACV",
  ...
  group_by(sexo, grupedad_10, compli, presencia) %>% 
  summarise(recuento = n()) %>% 
  ungroup() %>% 
  group_by(sexo, grupedad_10, compli) %>% 
  mutate(perc = round(recuento/sum(recuento)*100, digits = 1)) %>% 
  ...
  left_join(DW_GBD_recorte,
            by = join_by(compli == `complicación crónica_Qualidiab`)) %>%
  
  mutate(DW_promedio = case_when(
    compli == "Sin complicaciones" ~ 0.049,
    .default = DW_promedio
  )) %>% 
  
  select(sexo, grupedad_10, `tipo_complicación crónica`, compli, perc, DW_promedio) %>% 
  
  rename("Sexo" = "sexo",
         "Grupo de edad" = "grupedad_10",
         "Tipo de complicación" = "tipo_complicación crónica",
         "Complicación" = "compli",
         "Frecuencia" = "perc",
         "DW" = "DW_promedio") %>%
  ungroup()
...
## Grupos de edad grandes ----
complic_frecyDW_gr <- qualidiab_2014_dm2 %>% 
  
  select(c(sexo, grupedad_gr, 
           complic, 
           complicaciones_iam_c, 
           complicaciones_ic,
           complicaciones_acv,
  ...
  group_by(sexo, grupedad_gr, compli, presencia) %>% 
  summarise(recuento = n()) %>% 
  ungroup() %>% 
  group_by(sexo, grupedad_gr, compli) %>% 
  mutate(perc = round(recuento/sum(recuento)*100, digits = 1)) %>% 
  ...
  select(sexo, grupedad_gr, `tipo_complicación crónica`, compli, perc, DW_promedio) %>% 
  
  rename("Sexo" = "sexo",
         "Grupo de edad" = "grupedad_gr",
         "Tipo de complicación" = "tipo_complicación crónica",
         "Complicación" = "compli",
         "Frecuencia" = "perc",
         "DW" = "DW_promedio") %>%
...
## Exportación de tabla de frecuencias y DW ----
write_xlsx(complic_frecyDW_10, "frec_DW_complic_sexo_10.xlsx")
write_xlsx(complic_frecyDW_gr, "frec_DW_complic_sexo_gr.xlsx")
